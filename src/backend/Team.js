...
router.get("/TeamInformationForDashbord", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    let query =
      "SELECT team.Team_id,team.Team_name,team.Email, team.Roles, team.Phone_number, team.Qualification, team.Skills,team.Profile_image,Project.Project_name FROM Team join Project on Project.Team_id=Team.Team_id or Team.Project_id=Project.Project_id  WHERE team.Tl_id = 7 and Team.Active='Active'";
    connection.query(query, tlid, (err, data) => {
      connection.release();
